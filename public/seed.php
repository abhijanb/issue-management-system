...
function seedIssues(Database $database)
{
    ...
    for ($i = 0; $i < 29; $i++) {
        ...
        $isDeleted = $assignee_id?0:rand(0, 1);
        ...
        $issue = [$i,$title, $description, $status, $priority, $assignee_id, $reporter_id, getRandomDate(), $isDeleted];
        $queryString = "INSERT INTO issues (id, title, description, status, priority, assignee_id, reporter_id, created_at, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $database->query($queryString, $issue);

        if($isDeleted===1){
            $queryString = "INSERT INTO issue_deletion_logs (issue_id, deleted_by) VALUES (?, ?)";
            $database->query($queryString, [$i, $reporter_id]);
        }
